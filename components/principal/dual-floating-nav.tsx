    ...
    <div className="relative" onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
      ...
      >
        {isActive && (
          <motion.span
            layoutId="activePill"
            className="absolute inset-0 rounded-2xl bg-brand-600 shadow-[0_0_16px_rgb(var(--brand-600)/0.5)]"
            transition={{ type: "spring", stiffness: 350, damping: 30 }}
          />
        )}
        <tab.icon
          className={cn("relative z-10 h-5 w-5 transition-colors", isActive ? "text-white" : "text-cream/50 group-hover:text-cream")}
        />
      </button>
      ...
      <AnimatePresence>
        {hovered && (
          <motion.span
            ...
            className={cn(
              "pointer-events-none absolute top-1/2 z-20 -translate-y-1/2 whitespace-nowrap rounded-lg border border-white/20 bg-white/70 px-2.5 py-1.5 text-xs font-medium text-espresso shadow-lg backdrop-blur-md dark:border-white/10 dark:bg-midnight-card/70 dark:text-cream",
  ...
  return (
    ...
    <div className={cn("fixed top-1/2 z-50 hidden -translate-y-1/2 md:block", side === "left" ? "left-6" : "right-6")}>
      <motion.nav
        ...
        className="flex flex-col space-y-2 rounded-3xl border border-white/10 bg-[#1C1512]/60 p-2.5 shadow-[0_8px_32px_0_rgba(0,0,0,0.36)] backdrop-blur-2xl"
      >
        {tabs.map((tab) => (
