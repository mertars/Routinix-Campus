...
export function TercihRobotuTab() {
  ...
  if (track === "genel") {
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center gap-2 rounded-3xl border border-hairline bg-white/70 p-8 text-center backdrop-blur-sm dark:border-white/10 dark:bg-midnight-card/70">
  ...
  return (
    <div className="space-y-4">
      ...
      {(["garanti", "ideal", "hayal"] as PreferenceCategory[]).map((cat) => {
        ...
        return (
          <motion.div key={cat} whileHover={{ scale: 1.005, y: -2 }} className="rounded-3xl border border-hairline bg-white/70 p-5 shadow-sm backdrop-blur-sm dark:border-white/10 dark:bg-midnight-card/70">
