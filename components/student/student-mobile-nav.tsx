...
import { Home, GraduationCap, Timer, Grid3x3, X, Search } from "lucide-react";
import type { NavTab } from "@/components/principal/floating-nav";
import { cn } from "@/lib/utils";

const AKADEMIK_IDS = ["net-tracker", "xray", "gap-closing", "weekly-schedule"];
const CALISMA_IDS = ["pomodoro", "etut", "ask-question", "guidance"];

function BottomSheetShell({ isOpen, onClose, children }: { isOpen: boolean; onClose: () => void; children: React.ReactNode }) {
  return (
    <AnimatePresence>
      {isOpen && (
        <>
...
export function StudentMobileNav({
  ...
  const calismaTabs = useMemo(() => CALISMA_IDS.map((id) => allTabs.find((t) => t.id === id)).filter((t): t is NavTab => !!t), [allTabs]);

  const [openSheet, setOpenSheet] = useState<"akademik" | "calisma" | "all" | null>(null);
  ...
  const isCalismaActive = CALISMA_IDS.includes(activeTab);
  const isOtherActive = !isOverviewActive && !isAkademikActive && !isCalismaActive;
    ...
    <>
      ...
      >
        <button onClick={() => handleSelect("overview")} className="relative flex min-h-[60px] flex-col items-center justify-center gap-1 py-2">
          {isOverviewActive && (
            <motion.span layoutId="studentMobilePill" className="absolute inset-x-3 top-1 h-0.5 rounded-full bg-brand-600" transition={{ type: "spring", stiffness: 400, damping: 32 }} />
          )}
          <Home className={cn("h-5 w-5", isOverviewActive ? "text-brand-600" : "text-espresso/50 dark:text-cream/40")} />
        ...
        <button
          onClick={() => setOpenSheet(openSheet === "calisma" ? null : "calisma")}
          className="relative flex min-h-[60px] flex-col items-center justify-center gap-1 py-2"
        >
          {isCalismaActive && (
            <motion.span layoutId="studentMobilePill" className="absolute inset-x-3 top-1 h-0.5 rounded-full bg-brand-600" transition={{ type: "spring", stiffness: 400, damping: 32 }} />
          )}
          <Timer className={cn("h-5 w-5", isCalismaActive ? "text-brand-600" : "text-espresso/50 dark:text-cream/40")} />
          <span className={cn("text-[9px] leading-tight", isCalismaActive ? "font-semibold text-brand-600" : "text-espresso/50 dark:text-cream/40")}>
            Çalışma & Etüt
          </span>
        </button>
      ...
      <BottomSheetShell isOpen={openSheet === "akademik"} onClose={() => setOpenSheet(null)}>
        <div className="min-h-0 flex-1 overflow-y-auto px-5 pb-6 pt-3">
          <div className="mb-3 flex items-center justify-between">
            <h3 className="text-sm font-semibold text-espresso dark:text-cream">Akademik & Sınav</h3>
      ...
      <BottomSheetShell isOpen={openSheet === "calisma"} onClose={() => setOpenSheet(null)}>
        <div className="min-h-0 flex-1 overflow-y-auto px-5 pb-6 pt-3">
          <div className="mb-3 flex items-center justify-between">
            <h3 className="text-sm font-semibold text-espresso dark:text-cream">Çalışma & Etüt</h3>
            <button onClick={() => setOpenSheet(null)} className="flex h-8 w-8 items-center justify-center rounded-full text-espresso-muted hover:bg-cream-card dark:text-cream/50 dark:hover:bg-white/10">
              <X className="h-4 w-4" />
            </button>
          </div>
