...
export function InstitutionDetailModal({ institutionId, onClose }: { institutionId: string | null; onClose: () => void }) {
  ...
  return (
    <>
            ...
            <div className="max-h-96 space-y-1.5 overflow-y-auto">
              {rows.map((row) => (
                ...
                >
                  ...
                  <button
                    onClick={() => setPermanentDeleteTarget({ id: row.id, role: tab, name: row.name })}
                    aria-label={tab === "STUDENT" ? "Öğrenciyi kalıcı olarak sil" : "Öğretmeni kalıcı olarak sil"}
      ...
      <PermanentDeleteConfirmModal target={permanentDeleteTarget} onClose={() => setPermanentDeleteTarget(null)} onDeleted={loadAll} apiBase={apiBase} />
      <BulkImportWizard
