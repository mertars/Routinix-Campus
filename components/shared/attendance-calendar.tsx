...
// AYLIK YOKLAMA TAKVİMİ — veli ve öğrenci panelinin ORTAK ekranı.
//
// "Çocuğum bugün derse geldi mi?" (veli) ve "kaç gün devamsızlığım var?"
// (öğrenci) aynı veriyi soruyor. Panel eskiden yalnızca tek bir yüzde
// gösteriyordu; yüzde "dün gelmemiş" ile "üç haftadır gelmiyor"u aynı
// gösterir. Bu ekran günü güne, dersi derse gösterir.
//
// ⚠️ Tek fark VERİ KAYNAĞI: veli /api/parent/attendance/[id], öğrenci
// /api/student/attendance (kendi oturumundan). Bileşen kopyalanmadı,
// endpoint parametre olarak alınıyor — biri düzeltilince diğeri de düzelir.
export function AttendanceCalendar({
  endpoint,
  absentTitle,
}: {
  endpoint: (month: string) => string;
  absentTitle: (dayCount: number) => string;
}) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(endpoint(month));
    ...
    <div className="space-y-4">
      ...
      {!loading && absentDays.length > 0 && (
        <div className="rounded-2xl border border-red-500/30 bg-red-50 p-4 dark:bg-red-500/10">
          <p className="mb-2 text-sm font-semibold text-red-800 dark:text-red-300">{absentTitle(absentDays.length)}</p>
            ...
            {absentDays.map((d) => (
              <div key={d.date} className="flex flex-wrap items-center gap-2 text-xs">
                ...
                <span className="text-espresso-muted dark:text-cream/40">
                  {d.absentCount}/{d.totalCount} ders
                </span>
              </div>
            ))}
          </div>
