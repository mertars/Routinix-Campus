...
async function handleGet() {
  try {
    ...
    logger.error("recurring_expenses_list_failed", { error: error instanceof Error ? error.message : String(error) });
...
async function handlePost(request: NextRequest) {
  try {
    ...
    if (error instanceof AuthError) return authErrorResponse(error);
    logger.error("recurring_expense_create_failed", { error: error instanceof Error ? error.message : String(error) });
    return NextResponse.json({ error: "Beklenmeyen hata" }, { status: 500 });
  }
}
...
async function handlePatch(request: NextRequest) {
  try {
    ...
    if (error instanceof AuthError) return authErrorResponse(error);
    logger.error("recurring_expense_update_failed", { error: error instanceof Error ? error.message : String(error) });
    return NextResponse.json({ error: "Beklenmeyen hata" }, { status: 500 });
  }
}
