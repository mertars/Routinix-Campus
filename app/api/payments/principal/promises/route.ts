...
async function handleGet(request: NextRequest) {
  try {
    ...
    logger.error("promises_list_failed", { error: error instanceof Error ? error.message : String(error) });
...
async function handlePost(request: NextRequest) {
  try {
    ...
    if (error instanceof AuthError) return authErrorResponse(error);
    logger.error("promise_create_failed", { error: error instanceof Error ? error.message : String(error) });
    return NextResponse.json({ error: "Beklenmeyen hata" }, { status: 500 });
  }
}
...
async function handlePatch(request: NextRequest) {
  try {
    ...
    if (error instanceof AuthError) return authErrorResponse(error);
    logger.error("promise_close_failed", { error: error instanceof Error ? error.message : String(error) });
    return NextResponse.json({ error: "Beklenmeyen hata" }, { status: 500 });
  }
}
