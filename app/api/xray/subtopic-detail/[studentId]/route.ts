...
async function handleGet(request: NextRequest, { params }: { params: { studentId: string } }) {
  try {
    ...
    const attempts = await prisma.xrayPracticeAttempt.findMany({
      where: { studentId: params.studentId, subject, subtopicId, status: "COMPLETED" },
      orderBy: { completedAt: "desc" },
      select: {
        id: true,
        assignedAt: true,
        completedAt: true,
        answers: { select: { wasCorrect: true, question: { select: { prompt: true, correctAnswer: true, solution: true, checks: true, kazanimId: true } } } },
      },
    });
