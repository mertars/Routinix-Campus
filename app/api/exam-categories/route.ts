...
async function handleGet() {
  try {
    ...
    logger.error("exam_categories_list_failed", { error: error instanceof Error ? error.message : String(error) });
...
async function handlePost(request: NextRequest) {
  try {
    ...
    logger.error("exam_category_create_failed", { error: error instanceof Error ? error.message : String(error) });
    return NextResponse.json({ error: "Beklenmeyen hata" }, { status: 500 });
  }
}
