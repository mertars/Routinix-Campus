...
async function handleGet() {
  try {
    const exams = await prisma.exam.findMany({ orderBy: { examDate: "desc" } });
...
async function handlePost(request: NextRequest) {
  try {
    ...
    const exam = await prisma.exam.create({ data: { name: name.trim(), examDate: examDate ? new Date(examDate) : new Date() } });
