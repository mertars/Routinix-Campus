...
// Kasa/banka bakiyesi DÖRT kaynaktan beslenir:
//   + tahsil edilen gelir (Payment, COMPLETED)
//   − ödenmiş gider (Expense, PAID)
//   ± hesaplar arası virman (AccountTransfer)
//   − öğrenciye yapılan iade (EnrollmentCancellation.refundAmount)
...
// iade eklenirken bu merkezileştirme sayesinde tek dosya değişti.
...
export async function computeAccountBalances(institutionId: string): Promise<AccountBalanceRow[]> {
  const [accounts, incomes, paidExpenses, transfersOut, transfersIn, refunds] = await Promise.all([
    ...
    prisma.enrollmentCancellation.groupBy({
      by: ["refundAccountId"],
      where: { institutionId, refundAccountId: { not: null }, refundAmount: { gt: 0 } },
  ...
  const refundBy = new Map(refunds.map((r) => [r.refundAccountId as string, Number(r._sum.refundAmount ?? 0)]));
  ...
  return accounts.map((a) => ({
    ...
    type: a.type,
    balance:
      (incomeBy.get(a.id) ?? 0) - (expenseBy.get(a.id) ?? 0) - (outBy.get(a.id) ?? 0) + (inBy.get(a.id) ?? 0) - (refundBy.get(a.id) ?? 0),
  }));
}
